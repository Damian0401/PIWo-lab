...
const EstateFilters = ({ estates, setEstates }: IEstateFiltersProps) => {
    ...
    const allNumberOfBedrooms = [...Array(maxNumberOfBedrooms).keys()].map(x => x + 1);
    ...
    const handleBedroomsSelect = (event: ChangeEvent<HTMLSelectElement>): void => {
    ...
    const filterEstates = (): void => {
        ...
        if (selectedCity !== 'All') {
            filteredEstates = filteredEstates
                .filter((estate) => estate.city === selectedCity);
        }

        if (selectedBedrooms !== 'All') {
            filteredEstates = filteredEstates
                .filter((estate) => estate.bedrooms === Number(selectedBedrooms));
        }
    ...
    const cachedFilterEstates = useCallback(filterEstates,
        [estates, selectedCity, selectedBedrooms, searchQuery, isDescending, setEstates]
    );
