...
import EstateListItem from "./components/list-item/EstateListItem";
import { useState } from "react";
import EstateFilters from "./components/filters/EstateFilters";
...
const EstateList = ({ estates }: IEstateListProps) => {
  ...
  const estateList = filteredEstates.map((estate) => (
    <EstateListItem key={estate.id} estate={estate} />
  ));

  return (
    <div className={styles.container}>
      <EstateFilters estates={estates} setEstates={setFilteredEstates} />
      <div className={styles.listContainer}>{estateList}</div>
    </div>
  );
};
